...
def home():
    ...
    for i in card_task:
        ...
        for j in tmp_task:
            c_task += 1
    if c_task > 0:
        return render_template("home.html", clist=ctask, card_list=card_task, card_name=card_name, username=user, msg=msg)
    elif c_task == 0 and count > 1:
        return render_template("home_noList.html", card_list=card_task, username=user)
    else:
        if request.method == "GET":
            return render_template("home_noCard.html", username=user, msg=msg)
